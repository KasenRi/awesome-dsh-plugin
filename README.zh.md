...
- [KasenRi/dsh-orbit-browser-plugins#browser](https://github.com/KasenRi/dsh-orbit-browser-plugins/tree/main/packages/browser) — DeepSeek Harness 的受控浏览器自动化：agent_browser 工具与 BrowserAutomationService 通过 agent-browser CLI 驱动真实 Chromium / Chrome 页面（或指定 CDP 端点），支持 allowedDomains 域名收敛与单次调用输出上限。
...
- [KasenRi/dsh-orbit-browser-plugins#orbit](https://github.com/KasenRi/dsh-orbit-browser-plugins/tree/main/packages/orbit) — DeepSeek Harness 的确定性工程编排运行时：orbit_controller 工具与 OrbitService 以 Commander / Executor / Smart Watchdog 子代理执行 PLAN、EXECUTE、EVALUATE、有界修正与运行时恢复，持久状态保存在 <project>/.cx/state.json。
